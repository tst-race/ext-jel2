...
// This was expected to be 1, but there is a buffer overrun in this
// code that causes a SIGABRT.  For now, overallocate:
...
int ijel_set_ecc_blocklen(int new_len) {
  ...
  // printf("ijel_set_ecc_blocklen:  block_len = %d, max_mlen = %d\n", block_len, max_mlen);
  return max_mlen;
}
...
unsigned char *ijel_encode_ecc(unsigned char *msg, int msglen, int *outlen) {
  ...
  n_out = HACK * nblocks * block_len;     /* Number of bytes we need for ECC output */
  out = calloc((size_t) (n_out+1), 1);       /* Allocate and add 1 extra byte for NUL */
  ...
  *outlen = 0;      /* Total output length. */
  ...
  for (i = 0; i < nblocks; i++) {
    ...
    if (in_len > 0){
      int count = msgchunk < in_len ? msgchunk : in_len;
      memcpy(message+1, in, (size_t) count);
    }
...
unsigned char *ijel_decode_ecc(unsigned char *ecc, int ecclen, int *msglen) {
  ...
  nblocks = HACK * ecclen / block_len; /* The number of ECC blocks in the buffer. */
  ...
  out = calloc((size_t) nblocks+1, (size_t) block_len);  /* Allocate buffer */
  next_out = out;         /* Initialize the output buffer position */
  limit = out + nblocks * block_len;
  ...
  for (i = 0; i < nblocks && !done; i++) {
	...
	*msglen = 0;
	...
	return NULL;
      ...
      memcpy(next_out, in+1, (size_t) mlen);
...
int ijel_capacity_ecc(int nbytes) {
  int num_ecc_blocks = (nbytes / block_len);  /* conservative estimate */

  return num_ecc_blocks * (max_mlen-1);  /* We can fit this many plaintext bytes AFTER RS coding. */
}
...
int ijel_ecc_sanity_check(unsigned char *msg, int msglen) {
  int i;
  size_t buffer_sz = (size_t) (msglen+1);
  ...
  memcpy(buffer1, msg, (size_t) msgchunk);
...
unsigned char *ijel_encode_ecc_nolength(unsigned char *msg, int msglen, int *outlen) {
  ...
  *outlen = 0;      /* Total output length. */
  ...
  for (i = 0; i < nblocks && in_len > 0; i++) {
    ...
    if (in_len > 0){
      memcpy(message, in, (size_t) (msgchunk > in_len ? in_len : msgchunk));
    }
...
unsigned char *ijel_decode_ecc_nolength(unsigned char *ecc, int ecclen, int length) {
  ...
  out = calloc((size_t) nblocks, (size_t) block_len);  /* Allocate buffer */
  next_out = out;                    /* Initialize the output buffer position */
  ...
  for (i = 0; i < nblocks; i++) {
    ...
    memcpy(next_out, in, (size_t) msgchunk);
