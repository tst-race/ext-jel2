...
double* ijel_spectrum(jel_config *cfg) {
  ...
  out = calloc(1, DCTSIZE2 * sizeof(double));
  for (i = 0; i < DCTSIZE2; i++) out[i] = 0.0;
  ...
  /* If not already specified, find a set of frequencies suitable for
     embedding 8 bits per MCU.  Use the destination object, NOT cinfo,
     which is the source: */
  // JEL_LOG(cfg, 2, "ijel_spectrum: fspec->nfreqs = %d; fspec->maxfreqs = %d\n", fspec->nfreqs, fspec->maxfreqs);
  ...
  for (blk_y = 0; blk_y < bheight;
	...
	for (i = 0; i < DCTSIZE2; i++) {
	  v = (double) (mcu[i]); // * qtable->quantval[i]);
	  out[i] += v * v;
	}
      }
    }
...
/*
 * Some helpers to insert and extract bits:
 */
...
int ijel_insert_density(jel_config *cfg,  jelbs *stream, JCOEF *mcu, int chan) {
  ...
  /* for each byte in the MCU, insert it at the appropriate
     frequencies: */
  if (cfg->freqs.maxfreqs < 4) {
    JEL_LOG(cfg, 2, "ijel_insert_density: Need at least 4 frequencies to encode density!");
    return 0;
  }
  ...
  for (j = 0; j < 4; j++) {   // Always use 4 frequencies for density
    ...
    if (eom) val = 0;
    //    mcu[ flist[j] ] = val - 1;
    if (!cfg->set_lsbs) mcu[ flist[j] ] = XFORM(val);
    else {
      uint32_t mask = (1 << 2) - 1;
      uint32_t mval = mcu[ flist[j] ];
      mcu[ flist[j] ] = ( mval & ~mask ) | ( val & mask );
    }
  }
...
int ijel_insert_bits(jel_config *cfg,  jelbs *stream, JCOEF *mcu) {
  ...
  if ( cfg->mcu_flag[ idx ] ) {
	...
	uint32_t mval = mcu[ flist[j] ];
	uint32_t mask = (1 << cfg->bits_per_freq) - 1;
	mcu[ flist[j] ] = ( mval & ~mask ) | ( val & mask );
      }
...
int ijel_stuff_message(jel_config *cfg, int chan) {
  ...
  */
  ...
  if ( cfg->mcu_density == -1 ) {
    ...
    cfg->nmcus = (int) floor( (cfg->mcu_density * cfg->maxmcus) / 100.0 );
    JEL_LOG(cfg, 2, "ijel_stuff_message: MCU density has been auto-computed.  New values: density=%d, nmcus = %d\n", cfg->mcu_density, cfg->nmcus);
  }
  ...
  for (blk_y = 0; blk_y < bheight; blk_y += compptr->v_samp_factor) {
	...
	} else JEL_LOG(cfg, 3, "ijel_stuff_message: skipping MCU %d because nbits_in (%d) >= msg_nbits (%d)\n",
		       all_mcus, nbits_in, msg_nbits);
	all_mcus++;
      }
    }
...
int ijel_extract_density(jel_config *cfg,  jelbs *stream, JCOEF *mcu, int chan) {
  ...
  for (j = 0; j < nfreqs; j++) {
    ...
    if (!cfg->set_lsbs)  val = INVXFORM(mcu[ flist[j] ]);
    else val = mcu[ flist[j] ];
...
int ijel_unstuff_message(jel_config *cfg, int chan) {
  ...
  JEL_LOG(cfg, 2, "ijel_unstuff_message: last byte in message = %d, last byte in bitstream = %d.\n", message[msg_nbytes-1], bs->msg[msg_nbytes-1]);
...
int ijel_copy_markers(jel_config *c)  {
  int count, write_marker;
  
  struct jpeg_compress_struct *cinfo = &(c->dstinfo);
  struct jpeg_decompress_struct *dinfo = &(c->srcinfo);
    
  jpeg_saved_marker_ptr marker_p;


  count = 0;
  if (!cinfo || !dinfo) {
    jel_log(c, "ijel_copy_markers: Source or destination compress structs are missing.  Doing nothing.");
    return count;
  }

  marker_p = dinfo->marker_list;
  while ( marker_p ) {
    write_marker = 1;
    ...
    if ( marker_p->marker == JPEG_APP0 && marker_p->data_length >= 14 &&
	 marker_p->data[0] == 0x4a &&
	 marker_p->data[1] == 0x46 &&
	 marker_p->data[2] == 0x49 &&
	 marker_p->data[3] == 0x46 &&
	 marker_p->data[4] == 0x00 ) 
      write_marker=0;

    /* skip Adobe (APP14) marker */
    if ( marker_p->marker == JPEG_APP0+14 && marker_p->data_length >= 12 &&
	 marker_p->data[0] == 0x41 &&
	 marker_p->data[1] == 0x64 &&
	 marker_p->data[2] == 0x6f &&
	 marker_p->data[3] == 0x62 &&
	 marker_p->data[4] == 0x65 ) 
      write_marker=0;

    if (write_marker) {
      count++;
      jpeg_write_marker(cinfo, marker_p->marker, marker_p->data, marker_p->data_length);
    }
    
    marker_p = marker_p->next;
  }
  return count;
}
