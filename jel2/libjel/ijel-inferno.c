...
void jelbs_describe( jel_config *cfg, jelbs *obj );
...
void jelbs_describe( jel_config *cfg, jelbs *obj ) {
  ...
  jel_vlog(cfg, 2, "bit:        %8d   bits\n", obj->bit);
  jel_vlog(cfg, 2, "nbits:      %8d   bits\n", obj->nbits);
  jel_vlog(cfg, 2, "bufsize:    %8d   bytes\n", obj->bufsize);
  jel_vlog(cfg, 2, "density:    %8d   percent\n", obj->density);
  jel_vlog(cfg, 2, "msgsize:    %8d   bytes\n", obj->msgsize);
  jel_vlog(cfg, 2, "checksum:   %8x\n", obj->checksum);
  //  printf("message:    %s\n", obj->message);
}
...
static int jelbs_validate_checksum(jel_config *cfg, jelbs *bs) {
  ...
  b ^= len[3];
  if (bs->checksum == b) {
    jel_vlog(cfg, 2, "jelbs_validate_checksum: jelbs header checksums match: %2x == %2x.\n", b, bs->checksum);
    return 1;
  } else {
    jel_vlog(cfg, 2, "jelbs_validate_checksum: jelbs header checksum mismatch: %2x vs. %2x.\n", b, bs->checksum);
    return 0;
  }
}
...
jelbs *jelbs_create(jel_config *cfg, int size) {
  ...
  jel_vlog(cfg, 2, "jelbs_create: size=%d, allocated %d bytes.\n", size, obj->bufsize);
  jelbs_reset(obj);
  return obj;
}
...
int jelbs_get_bit(jelbs *obj, int k) {
  ...
  if (k < BITS_TO_PRINT) jel_vlog(obj->cfg, 2, "%d", val);
...
int jelbs_set_next_bit(jelbs *obj, int val) {
  ...
  if (obj->bit >= obj->nbits) {
    jel_vlog(obj->cfg, 3, "jelbs_set_next_bit: obj=0x%lx\n", (unsigned long) obj);
    jel_vlog(obj->cfg, 3, "jelbs_set_next_bit: Ran out of bits - bit pointer=%d but nbits=%d\n", obj->bit, obj->nbits);
    return -1;
  } else {
...
int *ijel_select_freqs( jel_config *cfg ) {
  ...
  if(jel_verbose){
    jel_vlog(cfg, 2, "ijel_select_freqs: returning %d frequencies: [", fspec->nfreqs);
    for (i = 0; i < fspec->nfreqs; i++) {
      jel_vlog(cfg, 2, " %d", fspec->in_use[i]);
    }
    jel_vlog(cfg, 2, "]\n");
  }
...
int *ijel_reset_freqs( jel_config *cfg ) {
  ...
  if(jel_verbose){
    jel_vlog(cfg, 2, "ijel_reset_freqs: selected %d frequencies: [", fspec->nfreqs);
    for (i = 0; i < fspec->nfreqs; i++) {
      jel_vlog(cfg, 2, " %d", fspec->in_use[i]);
    }
    jel_vlog(cfg, 2, "]\n");
...
int *ijel_permute_freqs( jel_config *cfg ) {
  ...
  if (cfg->seed) {
    ...
    if (jel_verbose) {
      jel_vlog(cfg, 3, "ijel_permute_freqs: [", fspec->nfreqs);
      for (i = 0; i < fspec->nfreqs; i++) {
	jel_vlog(cfg, 3, " %d", fspec->in_use[i]);
      }
      jel_vlog(cfg, 3, "]\n");
    }
  }
...
int ijel_max_mcus(jel_config *cfg, int component) {
  ...
  /* If not already specified, find a set of frequencies suitable for
     ...
     which is the source: */
  jel_vlog(cfg, 2, "ijel_max_mcus: fspec->nfreqs = %d; fspec->maxfreqs = %d\n", fspec->nfreqs, fspec->maxfreqs);
  if (!fspec->init) {
    ...
    jel_vlog(cfg, 2, "ijel_max_mcus: fspec->nfreqs is now %d; freqs = [", fspec->nfreqs);
    for (i = 0; i < fspec->maxfreqs; i++) jel_vlog(cfg, 2, "%d ", fspec->freqs[i]);
    jel_vlog(cfg, 2, "]\n");
  }
...
int ijel_create_mcu_map(jel_config *cfg, int compnum) {
  ...
  jel_vlog(cfg, 3, "ijel_create_mcu_map: Creating MCU map for compnum=%d.\n", compnum);
  ...
  jel_vlog(cfg, 3, "ijel_create_mcu_map: created and initialized MCU map of length %d.\n", max_mcus);
  return(max_mcus);
}
...
int ijel_select_mcus( jel_config *cfg, int compnum ) {
  ...
  jel_vlog(cfg, 3, "ijel_select_mcus maxmcus = %d, number of MCUs to use = %d, seed = %d\n",
  ...
  if (!cfg->seed) {
    ...
    jel_vlog(cfg, 3, "ijel_select_mcus: prn call count before = %d\n", cfg->prn_cache->ncalls);
    //    printf("MCUs: ");
    ...
    // jel_vlog(cfg, 3, "ijel_select_mcus mcu flags (%d): [", n);
    n = cfg->nmcus;
    for (j = 1; j < n; j++)
      cfg->mcu_flag[ cfg->mcu_list[j] ] = 1;

    jel_vlog(cfg, 3, "ijel_select_mcus: prn call count after = %d\n", cfg->prn_cache->ncalls);
  }
  return cfg->nmcus;
}
...
int ijel_print_energies(jel_config *cfg) {
  ...
  /* If not already specified, find a set of frequencies suitable for
     embedding 8 bits per MCU.  Use the destination object, NOT cinfo,
     which is the source: */
  jel_vlog(cfg, 2, "ijel_print_energies: fspec->nfreqs = %d; fspec->maxfreqs = %d\n", fspec->nfreqs, fspec->maxfreqs);
  if (!fspec->init) {
    /* If we explicitly set the output quality, then this will be
     * non-NULL, but otherwise we will need to get the tables from the
     * source: */
  ...
  if (jel_verbose) {
    jel_vlog(cfg, 3, "ijel_get_freq_indices: ");
    for (j = 0; j < fspec->nfreqs; j++) jel_vlog(cfg, 3, "%d ", fspec->freqs[j]);
    jel_vlog(cfg, 3, "\n");
  }
  ...
  if (fspec->nfreqs < 4) {
    if( debug ) {
      jel_vlog(cfg, 4, "ijel_print_energies: Sorry - not enough good frequencies at this quality factor.\n");
    }
    return 0;
  }
  ...
  jel_vlog(cfg, 4, "# min,max energy = %d, %d\n", min_energy, max_energy);
...
int ijel_image_capacity(jel_config *cfg, int compnum) {
  ...
  jel_vlog(cfg, 2, "ijel_image_capacity: bwidth = %d, bheight = %d, v_samp_factor = %d, capacity = %d.\n", bwidth, bheight, compptr->v_samp_factor, cap);
...
int ijel_capacity_iter(jel_config *cfg, int component) {
  ...
  /* If not already specified, find a set of frequencies suitable for
     embedding 8 bits per MCU.  Use the destination object, NOT cinfo,
     which is the source: */
  jel_vlog(cfg, 3, "ijel_capacity_iter: fspec->nfreqs = %d; fspec->maxfreqs = %d\n", fspec->nfreqs, fspec->maxfreqs);
  if (!fspec->init) {
    /* If we explicitly set the output quality, then this will be
     * non-NULL, but otherwise we will need to get the tables from the
     * source: */
    ...
    int ret = ijel_get_freq_indices(qtable, fspec->freqs, fspec->maxfreqs, fspec->nlevels);
    if ( fspec->nfreqs <= 0 ) fspec->nfreqs = ret;
    jel_vlog(cfg, 3, "ijel_capacity_iter: fspec->nfreqs is now %d; freqs = [", fspec->nfreqs);
    for (i = 0; i < fspec->maxfreqs; i++) jel_vlog(cfg, 3, "%d ", fspec->freqs[i]);
    jel_vlog(cfg, 3, "]\n");
  }
...
unsigned char *ijel_maybe_init_ecc(jel_config *cfg, unsigned char *raw_msg, int *msglen, int *ecc) {
  ...
  if (jel_getprop(cfg, JEL_PROP_ECC_METHOD) == JEL_ECC_RSCODE) {
    ...
    if (ijel_ecc_sanity_check(raw_msg, raw_msg_len)){
      jel_vlog(cfg, 3, "ijel_maybe_init_ecc: FYI, sanity check failed.\n");
    ...
    if (!message) {
      ...
      if (cfg->verbose > 1)
        jel_vlog(cfg, 3, "ijel_maybe_init_ecc: 1st 5 bytes of ECC data = %d %d %d %d %d\n", 
                message[0], message[1], message[2], message[3], message[4]);
      
      if(jel_verbose){
        jel_vlog(cfg, 3, "ijel_maybe_init_ecc: ECC enabled, %d bytes of message encoded in %d bytes.\n", raw_msg_len, i);
      }
      *msglen = i;
      *ecc = 1;
    }
...
int ijel_insert_density(jel_config *cfg,  jelbs *stream, JCOEF *mcu, int chan) {
  ...
  jel_vlog(cfg, 3, "ijel_insert_density: Inserting density %d\n", stream->density);
  ...
  jel_vlog(cfg, 3, "ijel_insert_density: Done inserting density (%d bits) on channel %d\n", nbits, chan);

  // Now reinitialize the active MCU map to account for the change in
  // density:
  ijel_select_mcus(cfg, cfg->components[chan]);

  if (stream->bit < BITS_TO_PRINT) jel_vlog(cfg, 3, "|");
  return nbits;
}
...
int ijel_stuff_message(jel_config *cfg, int chan) {
  ...
  jel_vlog(cfg, 1, "ijel_stuff_message: msglen = %d\n", msglen);
  if(jel_verbose) {
    jel_vlog(cfg, 2, "ijel_stuff_message: 1st 5 bytes of plain text = %d %d %d %d %d\n", 
  ...
  jel_vlog(cfg, 1, "ijel_stuff_message: jelbs message size = %lu, msglen = %d, nbits=%d\n", bs->msgsize, msglen, bs->nbits);

  /* If not already specified, find a set of frequencies suitable for
     embedding.  Use the destination object, NOT cinfo, which is the
     source: */
  jel_vlog(cfg, 2, "ijel_stuff_message: cfg->bits_per_freq = %d; fspec->nfreqs = %d; fspec->maxfreqs = %d\n",
  ...
  if (!fspec->init) {
    ...
    int ret = ijel_get_freq_indices(qtable, fspec->freqs, fspec->maxfreqs, fspec->nlevels);
    if ( fspec->nfreqs <= 0 ) fspec->nfreqs = ret;
    jel_vlog(cfg, 2, "ijel_stuff_message: fspec->nfreqs is now %d; freqs = [", fspec->nfreqs);
    for (i = 0; i < fspec->maxfreqs; i++) jel_vlog(cfg, 2, "%d ", fspec->freqs[i]);
    jel_vlog(cfg, 2, "]\n");
  }
  ...
  if ( debug && jel_verbose ) {     /* Do we want to use verbosity levels? */
    jel_vlog(cfg, 1, "(:components #(");
    for (i = 0; i < fspec->nfreqs; i++) jel_vlog(cfg, 1, "%d ", fspec->freqs[i]);
    jel_vlog(cfg, 1, "))\n");
  }
  ...
  */
  ...
  jel_vlog(cfg, 1, "ijel_stuff_message: embedded length = %lu bytes\n", bs->msgsize);
  
  //  if ( cfg->nmcus == 0 ) {
  cfg->nmcus = (int) floor( (cfg->mcu_density * cfg->maxmcus) / 100.0 );
  jel_vlog(cfg, 2, "ijel_stuff_message: nmcus = %d = (int) floor( (%d * %d) / 100.0 )\n",
  ...
  if ( cfg->mcu_density == -1 ) {
    ...
    jel_vlog(cfg, 2, "jel_stuff_message: bitstream_nbits=%f ; maxbits = %f\n", bitstream_nbits, maxbits);
  ...
  if (jel_verbose) {
    jel_vlog(cfg, 2, "ijel_stuff_message:  >>>>   bitstream before embedding:\n");
    jelbs_describe(cfg, bs);
  }
  ...
  if (!cfg->embed_bitstream_header) {
    first = FALSE;
    jel_vlog(cfg, 1, "ijel_stuff_message: Not embedding the header, just raw bits.\n");
  }

  //  jelprn_reset(cfg->prn_cache);
  ijel_reset_freqs(cfg);
  ijel_permute_freqs(cfg);
  ...
  for (blk_y = 0; blk_y < bheight; blk_y += compptr->v_samp_factor) {
	...
	if (nbits_in < msg_nbits) {
	  ...
	  maybe_describe_mcu(cfg, mcu, all_mcus, nm, "Before");
	  ...
	  maybe_describe_mcu(cfg, mcu, all_mcus, nm, "After");
  ...
  if (jel_verbose) {
    jel_vlog(cfg, 1, "ijel_stuff_message:    END OF MAIN PROCESSING LOOP\n");
    jel_vlog(cfg, 2, "ijel_stuff_message:  <<<<<   bitstream after embedding:\n");
    jelbs_describe(cfg, bs);
  }
...
int ijel_extract_density(jel_config *cfg,  jelbs *stream, JCOEF *mcu, int chan) {
  ...
  jel_vlog(cfg, 2, "ijel_extract_density: Extracted density %d (%d bits) on channel %d\n", cfg->mcu_density, chan);
...
int ijel_unstuff_message(jel_config *cfg, int chan) {
  ...
  jel_vlog(cfg, 1, "ijel_unstuff_message: fspec->nfreqs = %d; fspec->maxfreqs = %d\n", fspec->nfreqs, fspec->maxfreqs);
  if (!fspec->init) {
    if (compnum == YCOMP) j = 0;
    else j = 1;
  ...
  jel_vlog(cfg, 3, "ijel_unstuff_message: Creating new bitstream object 0x%lx with a capacity of %d.\n",
  ...
  jel_vlog(cfg, 2, "ijel_unstuff_message: msg_nbytes=%d, cfg->len=%d\n",
	   msg_nbytes, cfg->capacity[chan]);
  ...
  if (!cfg->embed_bitstream_header) {
    ...
    jel_vlog(cfg, 1, "ijel_unstuff_message: Not checking for bitstream header.  Density = %d, msgsize = %d\n", bs->density, bs->msgsize);
  }

  jel_vlog(cfg, 2, "ijel_unstuff_message:  >>>>>   bitstream 0x%lx before extraction:\n", (unsigned long) bs);
  jelbs_describe(cfg, bs);
  ...
  for (blk_y = 0; blk_y < bheight; blk_y += compptr->v_samp_factor) {
	...
	if (nbits_out < msg_nbits) {
	  ...
	  if (!first)  {
	    nb = ijel_extract_bits(cfg, bs, mcu);
	    // if (nb <= 0)  jel_vlog(cfg, 5, "ijel_unstuff_message: ijel_extract_bits returned nb=%d for mcu=%d\n", nb, mcu_count);
	  } else {
	    jel_vlog(cfg, 2, "ijel_unstuff_message: about to extract density: \n");
	    ...
	    //	  jel_vlog(cfg, "ijel_unstuff_message:  <<<<<   bitstream after density extraction:\n");
	    //	  jelbs_describe(cfg, bs);
	    first = false;
	  }
	  ...
	  if ( cfg->debug_mcu == -2 || ( cfg->mcu_flag[ all_mcus ] && cfg->debug_mcu == nm ) ) {
	    if (cfg->mcu_flag[ all_mcus ]) printf("===== After embedding: MCU %d (ACTIVE) =====\n", all_mcus);
	    else printf("===== After embedding: MCU %d  =====\n", all_mcus);
	    ijel_print_mcu(cfg, mcu, 1);
	    printf("^^^^^^^^^^^^^^^^^^\n");
	  }
	  ...
	  nbits_out += nb;
	  if ( nb > 0 ) mcu_count++;
	  //	jel_vlog(cfg, "ijel_unstuff_message: nbits_out = %d\n", nbits_out);

	  if ( 0 && !got_length ) {
	    jel_vlog(cfg, 2, "ijel_unstuff_message:  =====   bitstream before got_length:\n");
	    jelbs_describe(cfg, bs);
	  ...
	  if ( !got_length && jelbs_got_length( bs ) ) {
	    got_length = 1;
	    if ( jel_verbose ) {
	      jel_vlog(cfg, 3, "ijel_unstuff_message: got_length = %d ; jelbs_got_length(bs) = %d\n", got_length, jelbs_got_length(bs));
	    }
	
	    msg_nbytes = jelbs_get_length( bs );

	    if (!cfg->embed_bitstream_header || jelbs_validate_checksum( cfg, bs ) ) {
	      jel_vlog(cfg, 3, "ijel_unstuff_message: checksum OK in bitstream (%x)\n", bs->checksum);
	    } else {
	      jel_log(cfg, "ijel_unstuff_message: invalid checksum in bitstream (%x)\n", bs->checksum);
	      jel_vlog(cfg, 2, "ijel_unstuff_message:  <<<<<   bitstream at invalid checksum:\n");
	      jelbs_describe(cfg, bs);
	      jelbs_destroy(&bs);
	      ijel_destroy_mcu_map(cfg);
	      return JEL_ERR_CHECKSUM;
	    ...
	    jel_vlog(cfg, 2, "ijel_unstuff_message: Got length!  msg_nbytes = %d\n", msg_nbytes);
	    ...
	    if (msg_nbytes <= 0 || msg_nbytes > max_nbytes) {
	      jel_log (cfg, "ijel_unstuff_message: bogus value for msg_nbytes %d %d \n", msg_nbytes, max_nbytes);
	      jelbs_destroy(&bs);
	      ijel_destroy_mcu_map(cfg);
	      return JEL_ERR_MSG_OVERFLOW;
	    }
  ...
  if ( jel_verbose ) {
    jel_vlog(cfg, 1, "ijel_unstuff_message:    END OF MAIN PROCESSING LOOP\n");
    jel_vlog(cfg, 1, "ijel_unstuff_message: Processed %d MCUs.\n", all_mcus);
    jel_vlog(cfg, 2, "ijel_unstuff_message:  <<<<<   bitstream after extraction:\n");
    jelbs_describe(cfg, bs);

    jel_vlog(cfg, 2, "ijel_unstuff_message: embedded length = %d bytes\n", msg_nbytes);
  }
  ...
  if (fDoECC) {
    ...
    jel_vlog(cfg, 2, "ijel_unstuff_message: truek = %d, k = %d, %d\n", truek, k, msg_nbytes);
    ...
    jel_vlog(cfg, 2, "ijel_unstuff_message: ijel_ecc_length(%d) => %d\n", k, truek);
    jel_vlog(cfg, 2, "ijel_unstuff_message: 1st 5 bytes of ECC data = %d %d %d %d %d\n", 
	    message[0], message[1], message[2], message[3], message[4]);
    ...
    if (raw) {
      jel_vlog(cfg, 2,  "ijel_unstuff_message: ECC enabled, %d bytes of ECC data decoded into %d bytes of message.\n", k, i);
      ...
      jel_vlog(cfg, 3, "ijel_unstuff_message: 1st 5 bytes of plain text = %d %d %d %d %d\n", 
	       raw[0], raw[1], raw[2], raw[3], raw[4]);
  ...
  jel_vlog(cfg, 2, "ijel_unstuff_message: msg_nbytes=%d\n", msg_nbytes);
  jel_describe(cfg, 2);
...
int ijel_image_capacity(jel_config *cfg) {
  ...
  jel_vlog(cfg, 1, "bwidth = %d, bheight = %d, v_samp_factor = %d, capacity = %d.\n", bwidth, bheight, compptr->v_samp_factor, cap);
